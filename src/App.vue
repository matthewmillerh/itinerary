...
<template>
  <div v-if="loggedIn" class="wrapper pl-3">
    <header>
  ...
  <!-- Content filter bar -->
  <div class="max-w-3xl rounded-lg bg-gray-200 ml-auto mr-auto h-12 p-2 mt-3 flex items-center z-20 relative">
    <!-- Countries filter -->
    <div class="h-7 flex items-center">
      <button class="text-sm h-7 pl-3 pr-3 bg-white rounded-3xl font-semibold" @click="filterCountries">
    ...
    <div class="h-7 flex items-center ml-auto order-2">
      <button class="text-sm p-2 bg-white rounded-lg font-semibold"><i class="fa-solid fa-globe"></i> Map View</button>
    </div>
    
  </div>
  <!-- Countries filter list -->   
  <div class="max-w-3xl h-10 bg-gray-300 -mt-3 rounded-xl ml-auto mr-auto z-0 relative transition-all" v-if="showCountries">
    <div class="flex pt-4 ml-3 items-centers">
